...
| Change a repo's selection later | Edit its config by hand | `npx agentspread setup` → reconfigure opens a PR |
...
| `.github/workflows/agentspread-release.yml` | *Actions → release*: calls agentspread's release workflow |
| `.github/workflows/agentspread-sync.yml` | *Actions → sync consumers*: rolls a tag out again, e.g. to one repo |
| `.github/workflows/agentspread-check.yml` | lints `groups/` and PR titles |
| `.github/dependabot.yml` | a PR when a new agentspread version is out |
| `package.json` | pins the same agentspread version for `npm run setup` |

Nothing in the content repo lists consumer repos. Sync finds them at run time as the repos under the content repo's owner with the `agentspread-consumer` topic.
...
`name` must match the folder name; `description` tells the agent when to load it. Subagents, commands and `hooks.json` use [rulesync's formats](https://github.com/dyoshikawa/rulesync). A hook that runs a script refers to it as `.agentspread/scripts/<group>/<file>`, and the file lives in `groups/<group>/scripts/<file>`; CI checks that it exists.
...
The workflows' built-in `GITHUB_TOKEN` can only touch `agentspread` itself; this App is the bot identity that opens PRs in the other repos.
...
1. **Create it.** Org: *Org settings → Developer settings → GitHub Apps → New GitHub App*. Personal account: *Settings → Developer settings → GitHub Apps → New GitHub App*.
   - **GitHub App name:** anything unique on GitHub, e.g. `<org>-agentspread`. It is the bot's display name and can be renamed later.
...
4. **Create a `release` environment** in `<org>/ai-config` (*Settings → Environments → New environment*). Under *Deployment branches*, choose *Selected branches* and add your release branch (`main`, or the `RELEASE_BRANCH` below). Then store both values as **environment** secrets and delete the `.pem`:
   ...
   gh secret set AGENTSPREAD_APP_PRIVATE_KEY --repo <org>/ai-config --env release < path/to/the-downloaded.private-key.pem
   ```
   The App can push to agentspread and open PRs everywhere; keeping its key in an environment limited to the release branch stops any other branch from using it.
5. **Install it.** Creating the App does not install it. On the App's page choose *Install App* in the left menu → *Install* next to your org → pick *All repositories*, or *Only select repositories* including `agentspread` and every repo that will consume it. Skipping this makes every workflow run fail with `Not Found … get-a-user-installation`.
...
Non-interactive: `npx agentspread onboard web api --targets claudecode,codexcli --groups backend`. Repos are names under the content repo's owner, or `owner/name`.

For each repo it clones, runs [`adopt.sh`](scripts/adopt.sh) on a `chore/adopt-agentspread` branch and opens a PR. Repos already adopted, or with an open adoption PR, are skipped. The PR adds:

- `agentspread.json`: the pinned release and the repo's groups,
- `.agentspread/`: those groups' content at that release; never edited by hand,
- `rulesync.jsonc`: the chosen agents, and `.agentspread/` plus `.rulesync/` as its inputs,
- the generated agent files from the table above,
- `.github/workflows/agentspread-check.yml`, which regenerates everything and fails if the committed files differ (e.g. someone hand-edited them).

It also adds the `agentspread-consumer` topic to the repo; that topic is how releases find it, so don't add it by hand.

If the repo already had its own skills, subagents or commands, `adopt.sh` imports them into `.rulesync/`, where they stay the repo's own. Before merging, delete from there any that agentspread now ships (same name) and run `npx rulesync@16 generate --delete`.

If the repo already has hooks in `.claude/settings.json` and one of its groups ships hooks, adopt stops: generation would replace them. Move them into the agentspread group named after the repo, then re-run.

To adopt by hand instead, run `npx agentspread adopt <org>/ai-config` from the consumer repo's root.
...
```bash
gh workflow run agentspread-sync.yml --repo <org>/ai-config --ref <release branch> -f ref=$(gh release view --repo <org>/ai-config --json tagName -q .tagName)
```
...
| sync ([`sync.yml`](.github/workflows/sync.yml)) | in every repo with topic `agentspread-consumer`: copies its groups at the new tag into `.agentspread/`, regenerates, and opens or updates PR `chore/agentspread-sync` if anything changed, listing what. |
| consumer CI | [`agentspread-check.yml`](templates/consumer/consumer-ci.yml) regenerates and fails on drift |
...
**Change a repo's groups or agents:** `npm run setup` → *Reconfigure an adopted repo*, pick the repo, adjust the prefilled groups and agents. Or non-interactively: `npx agentspread reconfigure web --groups backend --targets claudecode,codexcli` (each flag sets the full list; `common` and the repo's own group are always kept). It opens a PR in that repo that keeps its agentspread release and regenerates for the new selection.
...
**In a consumer repo:**
- `.agentspread/` is agentspread's; `.rulesync/` is the repo's own and wins on a name clash, so a local skill with the same name overrides the shared one.
- To change a repo's groups or agents, use reconfigure (above). Editing `groups` in `agentspread.json` by hand also works; the next agentspread release applies it.
...
- Repo-specific hooks go in the agentspread group named after the repo, not in `.claude/settings.json`, which generation rewrites.
- `AGENTS.md` / `CLAUDE.md` stay yours; agentspread never touches them.

## Updating agentspread

The content repo pins agentspread twice: the `@vX.Y.Z` in its three workflow files (the engine that runs in Actions) and `agentspread` in `package.json` (the CLI behind `npm run setup`). Dependabot checks weekly and opens a PR for each when a new version is out; merge both. Release notes: [releases](https://github.com/sulhadin/agentspread/releases).

Nothing changes in consumer repos until the content repo's next release, which then runs the new engine.

## Security

What agentspread ships runs inside every developer's agent in every consumer: hooks and hook scripts execute commands, and a skill can pre-approve tools or run `` !`command` `` lines. Treat a merge to agentspread like a deploy:
...
`npm test` runs the suite. To release agentspread itself, merge conventional-commit PRs into `main` and run *Actions → publish*: it versions, publishes to npm through [trusted publishing](https://docs.npmjs.com/trusted-publishers) (no npm token), tags and creates the GitHub release that content repos pin.
