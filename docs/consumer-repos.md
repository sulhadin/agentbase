...
To adopt a single repo by hand, run `npx agentspread adopt <org>/ai-config` from its root.
...
- **Existing hooks** in `.claude/settings.json` stop adopt when a group ships hooks, since generation would replace them. Move them into the group named after the repo and re-run.
...
- `.agentspread/` and the generated folders are agentspread's. Never hand-edit them: the check rejects it and the next sync overwrites it.
