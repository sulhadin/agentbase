...
- agentspread's section in `CLAUDE.md` and/or `AGENTS.md`, if the groups ship instructions ([how it works](groups.md#instructions-agentsmd)),
...
- **rulesync's `rules` feature** writes `AGENTS.md` and `CLAUDE.md` whole and would drop agentspread's section, so adopt stops when it is on and the groups ship instructions. Move those rules into `AGENTS.md` or `CLAUDE.md` by hand and remove `rules` from `features` in `rulesync.jsonc`.
...
- `AGENTS.md` and `CLAUDE.md` stay yours except for the section between the `agentspread:start` and `agentspread:end` markers. Write your own notes above or below it; the check fails if the section itself is edited.
